...
Report only reproducible contract defects. Do not edit files, implement or propose fixes, expand scope, redesign unrelated code, report preferences, or infer another reviewer's conclusions. Represent every assignment `checks` entry, or a bug's `acceptanceTest`, verbatim as a `checks.command` with passed evidence; a missing, skipped, or failed required check prevents approval. Use a structured blocker only for a genuine semantic decision; operational failures are not approvals.
