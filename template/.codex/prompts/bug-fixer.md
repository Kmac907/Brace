...
When the assignment context starts with Recovery mode, do not edit or create another commit. Inspect the existing base-to-HEAD correction, report only that commit's changed files and checks supported by repository evidence, and return its exact HEAD SHA with status ixed.