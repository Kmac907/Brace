...
import new_worktree_ralph_project as bootstrap
...
class SafetyTests(unittest.TestCase):
    def test_stale_pull_request_head_is_rejected(self) -> None:
        value = [{
            ...
            "baseRefName": "ralph/integration", "baseRefOid": "c" * 40, "mergeCommit": None,
        }]
        ...
        with (
            ...
            self.assertRaisesRegex(common.RalphError, "different head SHA"),
        ):
            common.get_pull_request(".", config, "worktree/TASK-0001", "ralph/integration", "a" * 40)
    ...
    def test_multiple_exact_pull_requests_are_rejected(self) -> None:
        record = {
            ...
            "baseRefName": "ralph/integration", "baseRefOid": "c" * 40, "mergeCommit": {"oid": "d" * 40},
        }
        config = {"provider": "github", "github": {"repository": "owner/repo"}}
        with (
            patch.object(common, "get_repository_identity", return_value="owner/repo"),
            patch.object(common, "run_native", return_value=common.NativeResult(0, json.dumps([record, {**record, "number": 8}]))),
            self.assertRaisesRegex(common.RalphError, "Multiple pull requests"),
        ):
            common.get_pull_request(".", config, "worktree/TASK-0001", "ralph/integration", "a" * 40)
    ...
    def test_immutable_result_cannot_be_replaced(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            common.write_immutable_json(path, {"value": 1})
            with self.assertRaisesRegex(common.RalphError, "already exists with different content"):
    ...
    def test_documentation_authority_rejects_coordinator_files(self) -> None:
        ...
        with self.assertRaisesRegex(common.RalphError, "safe Markdown path"):
    ...
    def test_bootstrap_sections_preserve_existing_content_and_reject_duplicates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            bootstrap.add_section(path, "WORKTREE RALPH", "generated")
            self.assertEqual(path.read_text(encoding="utf-8"), "existing\n\n# BEGIN WORKTREE RALPH\ngenerated\n# END WORKTREE RALPH\n")
            with self.assertRaisesRegex(bootstrap.BootstrapError, "duplicate"):
                bootstrap.add_section(path, "WORKTREE RALPH", "again")
    ...
    def test_operational_blocker_does_not_enter_pm_workflow(self) -> None:
        with self.assertRaisesRegex(common.RalphError, "Operational blocker"):
